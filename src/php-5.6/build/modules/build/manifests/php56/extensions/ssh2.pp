class build::php56::extensions::ssh2 {
  ...
  file { '/tmp/libssh2-1.9.0.tgz':
    ensure => present,
    source => 'puppet:///modules/build/tmp/libssh2-1.9.0.tgz'
  }

  bash_exec { 'cd /tmp && tar xzf libssh2-1.9.0.tgz':
    require => File['/tmp/libssh2-1.9.0.tgz']
  }

  bash_exec { 'cd /tmp/libssh2-1.9.0 && ./configure':
    timeout => 0,
    require => Bash_exec['cd /tmp && tar xzf libssh2-1.9.0.tgz']
  }
